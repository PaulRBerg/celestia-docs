---
sidebar_label: Leap integration 
description: How you can add Celestia network parameters to Leap wallet.
---

# Leap integration with Celestia

This tutorial will go over how you can add Celestia network parameters to Leap wallet.

## Install Leap

Leap is a popular Cosmos-based wallet that allows anyone
to connect to Tendermint chains from their browser or phone.

Learn more and download Leap [here](https://www.leapwallet.io/).

Pick the browser or mobile device that you are using. Follow
the instructions to install Leap.

## Add Celestia network parameters

Click the Cosmos logo in the top corner of Leap wallet. Scroll down and
click "Add new chain"

<img src="/img/leap/leap.gif" width="300" />

You can
then add the following parameters:

* Chain Id: `mocha-3`
* Chain Name: `Mocha Testnet`
* New RPC URL: `https://rpc-mocha.pops.one`
* New REST URL: `https://api-mocha.pops.one`
* Address Prefix: `celestia`
* Native Denom: `utia`
* Coin Type: `118`
* Decimals: `6`
* Block explorer URL (optional): `https://testnet.mintscan.io/celestia-testnet`

Now, click `Add chain` and you will be able to view your Celestia
account balance and transactions in Leap wallet.

You'll see that you're connected to Mocha testnet.

<img src="/img/leap/leap.png" width="300" />
